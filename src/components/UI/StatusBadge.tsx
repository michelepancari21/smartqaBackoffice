...
const StatusBadge: React.FC<StatusBadgeProps> = ({ status, type = 'test' }) => {
  const getStatusConfig = (status: string | number, type: string) => {
    const configs: Record<string, Record<string | number, { bg: string; text: string; label: string }>> = {
      ...
      priority: {
        low: { bg: 'bg-gradient-to-r from-green-500/20 to-emerald-500/20 border-green-500/50', text: 'text-green-700 dark:text-green-400', label: 'Low' },
        medium: { bg: 'bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border-yellow-500/50', text: 'text-yellow-700 dark:text-yellow-400', label: 'Medium' },
        high: { bg: 'bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-500/50', text: 'text-orange-700 dark:text-orange-400', label: 'High' },
        critical: { bg: 'bg-gradient-to-r from-red-500/20 to-pink-500/20 border-red-500/50', text: 'text-red-700 dark:text-red-400', label: 'Critical' }
      }
    };
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${config.bg} ${config.text}`}>
      {config.label}
    </span>
  );
};
